/* -*- mode: c++; fill-column: 80; c-basic-offset: 4; indent-tabs-mode: nil -*- */
/****************************************************************
...
extern int no_sharp_line;
int chars_per_wd, gflag, no_sharp_line, protostatus;
int infertypes = 1;
...
static arg_info table[] = {
    ...
    f2c_entry ("nl", P_NO_ARGS, P_INT, &no_sharp_line, YES),
...
#endif
{
    ...
    if (c_output_name && ftn_files[0] && ftn_files[1]) {
    ...
    if (file_name && *file_name) {
        if (c_output_name) {
            coutput = c_output_name;
            cdfilename = c_output_name;
        } else if (debugflag != 1) {
            coutput = c_name(file_name, 'c');
            cdfilename = copys(coutput);
        } else {
            cdfilename = coutput;
        }
        if (Castargs1 >= 2) {
            proto_fname = copys(coutput);
